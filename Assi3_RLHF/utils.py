...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        # We use a 2-layer MLP. Input is 4D (state), hidden is 32, output is 1D (scalar reward).
        self.network = mlp(sizes=[4, 32, 1], activation=nn.ReLU, output_activation=nn.Identity)
    ...
    def predict_return(self, traj):
        ...
        # Pass the trajectory (sequence of states) through the network to get a sequence of step rewards
        step_rewards = self.network(traj)
        
        # Sum the step rewards to get the total cumulative return for the trajectory
        cumulative_reward = torch.sum(step_rewards)
