...
def learn_reward(reward_network, optimizer, training_inputs, training_outputs, num_iter, checkpoint_dir):
    ...
    for epoch in range(num_iter):
        ...
        for i in range(len(training_inputs)):
            trajA, trajB = training_inputs[i]
            label = training_outputs[i]
            
            # Convert arrays to tensors
            trajA_tensor = torch.tensor(np.array(trajA), dtype=torch.float32).to(device)
            trajB_tensor = torch.tensor(np.array(trajB), dtype=torch.float32).to(device)
            target_tensor = torch.tensor([label], dtype=torch.long).to(device)
            
            # Get cumulative returns
            retA = reward_network.predict_return(trajA_tensor)
            retB = reward_network.predict_return(trajB_tensor)
            
            # Format returns into a batched logit tensor of shape (1, 2)
            logits = torch.cat([retA.unsqueeze(0), retB.unsqueeze(0)]).unsqueeze(0)
            
            # Compute loss and update parameters
            loss = loss_criterion(logits, target_tensor)
        ...
        if (epoch + 1) % 10 == 0:
            print(f"Epoch {epoch+1}/{num_iter} | Loss: {epoch_loss/len(training_inputs):.4f}")
